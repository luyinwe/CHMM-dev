...
from Core.Data import extract_sequence, specialise_annotations, initialise_transmat, initialise_emissions
from Core.Constants import SOURCE_NAMES_TO_KEEP, CoNLL_INDICES, CoNLL_MAPPINGS,\
    CoNLL_LABELS, CoNLL_BIO, CoNLL_SOURCE_NAMES, CONLL_SRC_PRIORS
from Core.Util import set_seed_everywhere
from Core.Display import plot_tran_emis, plot_detailed_results


def less_lbs_test(args):
    ...
    train_annotations = list(map(specialise_annotations, train_annotations))
    ...
    for annotations in train_annotations:
        ...
        for src, annos in annotations.items():
            # if src not in SOURCE_NAMES_TO_KEEP:
            #     continue
            ...
            for span, values in annos.items():
                ...
                for lb, conf in values:
                    norm_lb = CoNLL_MAPPINGS.get(lb, lb)
                    if norm_lb in CoNLL_LABELS:
                        anno_dict[src][span] += ((norm_lb, conf),)
        train_annos.append(anno_dict)

    weak_lbs_all_src = [extract_sequence(s, a, label_indices=CoNLL_INDICES, ontonote_anno_scheme=False).numpy()
                        for s, a in zip(train_sents, train_annos)]

    train_weak_labels = [extract_sequence(s, a, SOURCE_NAMES_TO_KEEP,
                                          label_indices=CoNLL_INDICES, ontonote_anno_scheme=False)
                         for s, a in zip(train_sents, train_annos)]
    ...
    dev_annotations = list(map(specialise_annotations, dev_annotations))
    ...
    dev_annos = list()
    for annotations in dev_annotations:
        anno_dict = dict()
        for src, annos in annotations.items():
            if src not in SOURCE_NAMES_TO_KEEP:
                continue
            ...
            for span, values in annos.items():
                ...
                for lb, conf in values:
                    ...
                    if norm_lb in CoNLL_LABELS:
                        anno_dict[src][span] += ((norm_lb, conf),)
        dev_annos.append(anno_dict)

    dev_weak_labels = [extract_sequence(s, a, SOURCE_NAMES_TO_KEEP,
                                        label_indices=CoNLL_INDICES, ontonote_anno_scheme=False)
                       for s, a in zip(dev_sents, dev_annos)]
    ...
    trans_mat = torch.tensor(initialise_transmat(
        weak_lbs_all_src, label_set=CoNLL_BIO)[0], dtype=torch.float)
    emiss_mat = torch.tensor(initialise_emissions(
        weak_lbs_all_src, label_set=CoNLL_BIO, src_priors=CONLL_SRC_PRIORS
    )[0], dtype=torch.float)
    emiss_mat = emiss_mat[[CoNLL_SOURCE_NAMES.index(idx) for idx in SOURCE_NAMES_TO_KEEP], :, :]
