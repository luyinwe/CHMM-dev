...
class NeuralModule(nn.Module):
    def __init__(self,
        ...
        self.n_src = n_src
        self.n_obs = n_obs
        self.lstm_layer = nn.LSTM(d_emb, d_emb, num_layers=1, batch_first=True, bidirectional=True)
    ...
    def forward(self,
        ...
        batch_size, max_seq_length, d_emb = embs.size()

        packed = nn.utils.rnn.pack_padded_sequence(embs, seq_lengths, batch_first=True, enforce_sorted=False)
        lstm_emb, _ = self.lstm_layer(packed)
        lstm_emb, _ = nn.utils.rnn.pad_packed_sequence(lstm_emb, batch_first=True)
        lstm_emb = lstm_emb.view(batch_size, max_seq_length, 2, d_emb).sum(dim=-2)

        trans_temp = self.neural_transition(lstm_emb).view(
            batch_size, max_seq_length, self.n_hidden, self.n_hidden
        )
        nn_trans = torch.softmax(trans_temp / temperature, dim=-1)

        nn_emiss = torch.stack([torch.softmax(emiss(lstm_emb).view(
                batch_size, max_seq_length, self.n_hidden, self.n_obs
            ) / temperature, dim=-1) for emiss in self.neural_emissions]).permute(1, 2, 0, 3, 4)
        # emiss_temp = self.neural_emission(embs).view(
        #     batch_size, max_seq_length, self.n_src, self.n_hidden, self.n_obs
        # )
        # nn_emiss = torch.softmax(emiss_temp / temperature, dim=-1)
...
class NeuralHMM(nn.Module):
    ...
    def _initialize_states(self,
        ...
        nn_trans, nn_emiss = self.nn_module(embs=embs, seq_lengths=seq_lengths)
    ...
    def _expected_complete_log_likelihood(self, seq_lengths):
        ...
        # sum over j, k
        # TODO: it seems to be a bug... I should have used logtran[:, 1:, :, :] instead of the current one?
    ...
    def forward(self, emb, obs, seq_lengths):
        ...
        self._initialize_states(embs=emb, obs=obs, seq_lengths=seq_lengths)
