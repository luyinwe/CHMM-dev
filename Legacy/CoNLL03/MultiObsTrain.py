        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(data_loader)):
                ...
                for spans in pred_span:
                    ...
                    for k in spans:
                        norm_span = self.args.mappings.get(spans[k], spans[k])
                        if norm_span in self.args.tgt_lbs:
            ...
            results = get_results(batch_pred_span, batch_true_span, batch_sent, all_labels=self.args.tgt_lbs)
        return results
