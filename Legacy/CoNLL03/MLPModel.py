...
class NeuralModule(nn.Module):
    def __init__(self,
        ...
        self.n_src = n_src
        self.n_obs = n_obs
        self.trans_hidden_layer = nn.Linear(d_emb, d_emb)
        self.emiss_hidden_layer = nn.Linear(d_emb, d_emb)
    ...
    def forward(self,
        ...
        trans_hidden = self.dropout(F.gelu(self.trans_hidden_layer(embs)))
        emiss_hidden = self.dropout(F.gelu(self.emiss_hidden_layer(embs)))

        trans_temp = self.neural_transition(trans_hidden).view(
            batch_size, max_seq_length, self.n_hidden, self.n_hidden
        )
        nn_trans = torch.softmax(trans_temp / temperature, dim=-1)

        nn_emiss = torch.stack([torch.softmax(emiss(emiss_hidden).view(
