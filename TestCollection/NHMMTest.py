...
from Core.Data import extract_sequence, initialise_transmat, initialise_emissions, converse_ontonote_to_conll
from Core.Util import set_seed_everywhere
from Core.Display import plot_tran_emis, plot_dict


def neural_hmm_test(args):
    ...
    hmm_model.load_state_dict(torch.load(model_file)['model'])
    ...
    if args.save_scores:
        ...
        train_annos = annotate_data(
            model=hmm_model, text=exp_train_sents, embs=train_embs,
            obs=train_weak_lbs, lbs=train_lbs, args=args
        )
        score_name = f"{'.'.join(args.train_name.split('.')[:-1])}.{args.test_task}.scores"
        torch.save(train_annos, os.path.join(args.data_dir, score_name))
        
        dev_annos = annotate_data(
            model=hmm_model, text=exp_dev_sents, embs=dev_embs,
            obs=dev_weak_lbs, lbs=dev_lbs, args=args
        )
        score_name = f"{'.'.join(args.dev_name.split('.')[:-1])}.{args.test_task}.scores"
        torch.save(dev_annos, os.path.join(args.data_dir, score_name))
        
        test_annos = annotate_data(
            model=hmm_model, text=exp_test_sents, embs=test_embs,
            obs=test_weak_lbs, lbs=test_lbs, args=args
        )
        score_name = f"{'.'.join(args.test_name.split('.')[:-1])}.{args.test_task}.scores"
        torch.save(test_annos, os.path.join(args.data_dir, score_name))
        print("[INFO] Done")
